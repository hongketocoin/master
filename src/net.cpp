...
void ThreadGetMyExternalIP(void* parg)
{
    ...
    RenameThread("hongketocoin-ext-ip");
...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "HongKetoCoin " + FormatFullVersion();
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. HongKetoCoin is probably already running."), addrBind.ToString().c_str());
        else
...
{
    printf("StopNode()\n");
    GenerateHongKetoCoins(false, NULL);
    MapPort(false);
