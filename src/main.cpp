...
const string strMessageMagic = "HongKetoCoin Signed Message:\n";
...
void ThreadScriptCheck() {
    RenameThread("hongketocoin-scriptch");
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("HongKetoCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("HongKetoCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static HongKetoCoinMiner(CWallet *pwallet)
{
    printf("HongKetoCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("hongketocoin-miner");
    ...
    try { loop {
        ...
        printf("Running HongKetoCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("HongKetoCoinMiner terminated\n");
        throw;
    }
}

void GenerateHongKetoCoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&HongKetoCoinMiner, pwallet));
}
